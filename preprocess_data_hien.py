...
def cmu_hinglish_dog():
    ...
    for sp in splits:
        ...
        print(new_df)
        new_df.to_csv('./data/hi_en/cmu_hinglish_dog_{}_mt_en-hi_en.csv'.format(sp))
...
def CALCS_mt_enghinglish():
    ...
    for sp in splits:
        ...
        if sp != 'test':
            ...
            new_df = pd.DataFrame(data, columns=["translation"])
            print(new_df)
            new_df.to_csv('./data/hi_en/CALCS_hinglish_{}_mt_en-hi_en.csv'.format(sp))
        else:
            eng = lines
            ...
            print('df', df.head())
            df.to_csv('./data/hi_en/CALCS_hinglish_{}_mt_en-hi_en.csv'.format(sp))
            
def create_mt_data():
    df1 = pd.read_csv('./data/hi_en/cmu_hinglish_dog_train_mt_en-hi_en.csv', index_col=False)
    df2 = pd.read_csv('./data/hi_en/CALCS_hinglish_train_mt_en-hi_en.csv', index_col=False)
    frames = [df1, df2]
    result = pd.concat(frames, ignore_index=True)
    result.drop('Unnamed: 0', axis=1, inplace=True)
    result.to_csv('./data/hi_en/train_mt_en-hi_en.csv')

    df1 = pd.read_csv('./data/hi_en/cmu_hinglish_dog_validation_mt_en-hi_en.csv', index_col=False)
    df2 = pd.read_csv('./data/hi_en/CALCS_hinglish_dev_mt_en-hi_en.csv', index_col=False)
    frames = [df1, df2]
    result = pd.concat(frames, ignore_index=True)
    result.drop('Unnamed: 0', axis=1, inplace=True)
    result.to_csv('./data/hi_en/val_mt_en-hi_en.csv')
