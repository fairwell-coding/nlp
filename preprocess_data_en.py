...
Sadly, not used :( Data cleaning didn't help specifically
'''
...
def cmu_hinglish_dog():
    ...
    for sp in splits:
        ...
        for hinglish, english in zip(clean_df['hi_en'], clean_df['en']):
            str1 = str({'prefix':'translate Hinglish to English: ','input': hinglish, 'target': english})
            
            data.append(str1)

        new_df = pd.DataFrame(data, columns=["translation"])
        print(new_df)
        new_df.to_csv('./data/en/cmu_hinglish_dog_{}_mt_hi_en-en.csv'.format(sp))

def CALCS_mt_enghinglish():
    splits = ['train', 'dev', 'test']
    for sp in splits:
        lines = open('./data/mt_enghinglish/{}.txt'.format(sp), 'r').readlines()
        ...
        if sp != 'test':
            ...
            new_df = pd.DataFrame(data, columns=["translation"])
            print(new_df)
            new_df.to_csv('./data/en/CALCS_hinglish_{}_mt_hi_en-en.csv'.format(sp))
        else:
            continue
    
def create_mt_data():
    df1 = pd.read_csv('./data/en/cmu_hinglish_dog_train_mt_hi_en-en.csv', index_col=False)
    df2 = pd.read_csv('./data/en/CALCS_hinglish_train_mt_hi_en-en.csv', index_col=False)
    frames = [df1, df2]
    result = pd.concat(frames, ignore_index=True)
    result.drop('Unnamed: 0', axis=1, inplace=True)
    result.to_csv('./data/en/train_mt_hi_en-en.csv')

    df1 = pd.read_csv('./data/en/cmu_hinglish_dog_validation_mt_hi_en-en.csv', index_col=False)
    df2 = pd.read_csv('./data/en/CALCS_hinglish_dev_mt_hi_en-en.csv', index_col=False)
    frames = [df1, df2]
    result = pd.concat(frames, ignore_index=True)
    result.drop('Unnamed: 0', axis=1, inplace=True)
    result.to_csv('./data/en/val_mt_hi_en-en.csv')
